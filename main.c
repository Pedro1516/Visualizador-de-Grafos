            ...
            if (CheckCollisionPointLine(mousepoint, a.startpos, a.endpos, 5) && CheckCollisionPointCircle(mousepoint, a.startpos, 20) == false && CheckCollisionPointCircle(mousepoint, a.endpos, 20) == false)
            {
                ...
                else
                {
                    *aresta_selec = i;
                    ...
                    if (grafo->arestas[*aresta_selec].cor.a == 0)
                    {
                        *aresta_selec = -1;
                        return 0;
                    }
...
void limpar_animacao(Grafo *grafo, BFSAnim *bfs, DFSAnim *dfs, AGMAnim *agm_anim)
{
    if (bfs->ativa)
    ...
    if (agm_anim->ativa)
    {
        agm_anim->ativa = false;
        free(agm_anim->AGM);
        agm_anim->primeira_iteracao = true;
        agm_anim->timer = 0;
    }
    agm_anim->AGM = NULL;
...
int *prim(Grafo *grafo, int vertice_inicial)
{
    int quant_visitado = 0, outro;
    int *agm = (int *)malloc(sizeof(int) * (grafo->quant_v - 1));
    int *vertices_visitados = (int *)calloc(grafo->quant_v, sizeof(int));

    vertices_visitados[vertice_inicial] = 1;
    quant_visitado++;

    while (quant_visitado < grafo->quant_v)
    {
        int menor = INT_MAX;
        int aresta_escolhida = -1;
        ...
        for (int i = 0; i < grafo->quant_v; i++)
        {
            ...
            {
                for (int j = 0; j < grafo->vertices[i].quant_a; j++)
                {
                    Aresta *a = &grafo->arestas[grafo->vertices[i].arestas[j]];
                    ...
                    if (!vertices_visitados[outro])
                    {
                        if (a->peso < menor)
                        {
                            menor = a->peso;
                            aresta_escolhida = grafo->vertices[i].arestas[j];
                            vertice_escolhido = outro;
                        }
                    }
                }
            }
        ...
        agm[quant_visitado - 1] = aresta_escolhida;
        vertices_visitados[vertice_escolhido] = 1;
        quant_visitado++;
    }
...
int *inicia_animacao_AGM(Grafo *grafo, int vertice_inicial)
{
    if (!grafo->ponderado)
    {
        printf("O grafo precisa ser ponderado para calcular a AGM\n");
        return NULL;
    }
    ...
    for (size_t i = 0; i < grafo->quant_a; i++)
    {
        grafo->arestas[i].cor = (Color){100, 100, 100, 0};
    }

    return AGM;
}

bool passo_anim_AGM(Grafo *grafo, AGMAnim *AGM, bool tema_atual)
{
    static int index = 0;
    if (AGM->primeira_iteracao)
    {
        index = 0;
        AGM->primeira_iteracao = false;
    }

    Color cor_agm = tema_atual ? (Color){0, 0, 0, 255} : (Color){255, 255, 255, 255};
    grafo->arestas[AGM->AGM[index]].cor = cor_agm;
    index++;

    return index >= grafo->quant_v - 1;
}
...
int main()
{
    ...
    add_button_menu(menu_botoes, RED, "Arvore Geradora Minima");
    ...
    while (!WindowShouldClose())
    {
        ...
        if (onButtonClickScroll(&menu_botoes->list_btn[12], mousepoint, menu_botoes->scrollY, menu_botoes->rect) && !moving_cam && menu_botoes->aberto) // excluir grafo
        {
            if (vertices_selecionados[0] != -1)
            {
                agm_anim.AGM = inicia_animacao_AGM(grafo_global, vertices_selecionados[0]);
                if (agm_anim.AGM != NULL)
                {
                    agm_anim.ativa = true;
                    agm_anim.primeira_iteracao = true;
                    agm_anim.timer = 0;
        ...
        if (menu_rgb_vertice.aberto)
        {
            ...
            case 0:
                ...
                if (val_r > 255)
                    val_r = 255;
                menu_rgb_vertice.colorpicker.cor_atual.r = val_r;
            ...
            case 1:
                ...
                if (val_g > 255)
                    val_g = 255;
                menu_rgb_vertice.colorpicker.cor_atual.g = val_g;
            ...
            case 2:
                ...
                if (val_b > 255)
                    val_b = 255;
                menu_rgb_vertice.colorpicker.cor_atual.b = val_b;
            ...
            case 3:
                ...
                if (val_a > 255)
                    val_a = 255;
                menu_rgb_vertice.colorpicker.cor_atual.a = val_a;
            ...
            {
                menu_rgb_vertice.colorpicker.cor_atual.r = atoi(menu_rgb_vertice.r.buffer);
                menu_rgb_vertice.colorpicker.cor_atual.g = atoi(menu_rgb_vertice.g.buffer);
                menu_rgb_vertice.colorpicker.cor_atual.b = atoi(menu_rgb_vertice.b.buffer);
                menu_rgb_vertice.colorpicker.cor_atual.a = atoi(menu_rgb_vertice.a.buffer);
        ...
        if (menu_rgb_vertice.aberto && !moving_cam)
        {
            ...
            atualiza_cor_roda(&menu_rgb_vertice, mousepoint);
            atualiza_slider_rgb(&menu_rgb_vertice, mousepoint);
        }

        if (agm_anim.timer > 0.5f && agm_anim.ativa)
        {
            agm_anim.ativa = !passo_anim_AGM(grafo_global, &agm_anim, btn_mudar_tema.theme);
            agm_anim.timer = 0;
        }
        ...
        if (exibir_hud)
        {
            ...
            if (menu_rgb_vertice.aberto)
            {
                desenha_menu_rgb(menu_rgb_vertice, font);
            }
